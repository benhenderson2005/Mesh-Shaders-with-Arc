...
final class ArcMeshPipeline implements AutoCloseable {
	...
	private static final int REQUIRED_LOCAL_INVOCATIONS = 32;
	private static final int REQUIRED_OUTPUT_VERTICES = 128;
	private static final int REQUIRED_OUTPUT_PRIMITIVES = 64;
	...
	static ArcMeshPipeline create(final VulkanDevice device, final GpuFormat colorFormat, final GpuFormat depthFormat) {
		MeshLimits limits = queryLimits(device);
		limits.requireAtLeast(REQUIRED_LOCAL_INVOCATIONS, REQUIRED_OUTPUT_VERTICES, REQUIRED_OUTPUT_PRIMITIVES);
		...
		try {
			...
			MeshShadersWithArcClient.LOGGER.info(
				"Created VK_EXT_mesh_shader terrain pipeline (max direct workgroups={}, max vertices={}, max primitives={}, max storage range={} MiB)",
				limits.maxDrawWorkgroups,
				limits.maxOutputVertices,
				limits.maxOutputPrimitives,
				limits.maxStorageBufferRange / (1024L * 1024L)
			);
			return new ArcMeshPipeline(
				device,
				descriptorSetLayout,
				pipelineLayout,
				pipeline,
	...
	void draw(
		...
		while (dispatched < taskCount) {
			...
			try (MemoryStack stack = MemoryStack.stackPush()) {
				...
				pushConstants.putInt(0, firstTask + dispatched);
	...
	private void pushDescriptors(
		...
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			setBufferWrite(stack, writes.get(0), 0, VK12.VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, vertexBuffer.slice());
	...
	private static long createDescriptorSetLayout(final VulkanDevice device) {
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			VulkanUtils.crashIfFailure(
				...
				"Couldn't create Arc mesh descriptor layout"
			);
			return handle.get(0);
		}
	}
	...
	private static long createPipelineLayout(final VulkanDevice device, final long descriptorSetLayout) {
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			VulkanUtils.crashIfFailure(
				...
				"Couldn't create Arc mesh pipeline layout"
			);
			return handle.get(0);
		}
	}
	...
	private static long createGraphicsPipeline(
		...
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			VulkanUtils.crashIfFailure(
				...
				"Couldn't create Arc mesh terrain pipeline"
			);
			return handle.get(0);
		}
	}
	...
	private record MeshLimits(
		...
		private void requireAtLeast(final int invocations, final int vertices, final int primitives) {
			if (this.maxInvocations < invocations
				|| this.maxWorkgroupSizeX < invocations
				|| this.maxOutputVertices < vertices
				|| this.maxOutputPrimitives < primitives
				|| this.maxDrawWorkgroups < 1) {
				throw new IllegalStateException(
					"Mesh shader limits are too small: invocations=" + this.maxInvocations
						+ ", sizeX=" + this.maxWorkgroupSizeX
						+ ", vertices=" + this.maxOutputVertices
						+ ", primitives=" + this.maxOutputPrimitives
						+ ", directWorkgroups=" + this.maxDrawWorkgroups
				);
			}
		}
	}
